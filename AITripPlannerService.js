...
class AITripPlannerService {
    ...
    buildOptimizedPlan(title, description, type, city, days, budget, cityData, hotelTier, transportType, fitScore) {
        ...
        if (type === PlanType.ECONOMIC) targetBudget = budget * 0.65;      // Bütçenin %65'i (Örn 10.000 -> 6.500)
        else if (type === PlanType.BALANCED) targetBudget = budget * 0.88; // Bütçenin %88'i (Örn 10.000 -> 8.800)
        else if (type === PlanType.COMFORT) targetBudget = budget * 1.08;  // Bütçenin %108'i (Örn 10.000 -> 10.800)
        ...
        for (let i = 1; i <= days; i++) {
            // Premium plan bütçeyi doldurmak için daha fazla yer gezer (günde 5-6 yer)
            let activitiesPerDay;
            if (type === PlanType.COMFORT) {
                activitiesPerDay = Math.min(i === 1 ? 5 : 6, candidatePool.length);
            } else {
                activitiesPerDay = Math.min(i === 1 ? 3 : 4, candidatePool.length);
            }
            ...
            for (let j = 0; j < activitiesPerDay; j++) {
                ...
                for (let k = 0; k < candidatePool.length; k++) {
                    ...
                    if (isMealTime && candidate.category === "Yemek") {
                        ...
                        if (type === PlanType.ECONOMIC && mealCost > 500) continue;
                        if (type === PlanType.BALANCED && mealCost > 750) continue;
                    }
        ...
        // ─── Bütçe Bandı Kontrolü ───
        // Her plan tipi için kesin üst limitler:
        //   Eko:      max %70  (10.000 -> max 7.000)
        //   Dengeli:  max %85  (10.000 -> max 8.500)
        //   Premium:  max %110 (10.000 -> max 11.000)
        let maxAllowed;
        if (type === PlanType.ECONOMIC) maxAllowed = budget * 0.70;
        else if (type === PlanType.BALANCED) maxAllowed = budget * 0.92;
        else maxAllowed = budget * 1.10;

        // Bütçeyi aşıyorsa oteli ucuzlatarak sığdır
        while (totalEstimatedCost > maxAllowed && hotelIndex > 0) {
            hotelIndex--;
        ...
        // Hala aşıyorsa (en ucuz otel bile yetmiyorsa) tavana sabitle
        if (totalEstimatedCost > maxAllowed) {
            totalEstimatedCost = maxAllowed;
        }
